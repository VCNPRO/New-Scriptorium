...
export const Dashboard: React.FC<DashboardProps> = ({ recentManuscripts, onNewTranscription, onSelectManuscript }) => {
  ...
  const stats = {
      ...
      entities: recentManuscripts.reduce((acc, m) => acc + (m.analysis ? (m.analysis.entities.people.length + m.analysis.entities.locations.length) : 0), 0),
      alerts: recentManuscripts.filter(m => m.analysis?.qualityAlerts?.length).length
  };

  const filteredManuscripts = recentManuscripts
    .filter(m => {
        if (!searchValue && filterType !== 'status') return true;
        ...
        case 'entity': 
            ...
            return (m.analysis.entities.people.some(p => p.toLowerCase().includes(term)) || 
                    m.analysis.entities.locations.some(l => l.toLowerCase().includes(term)));
        case 'typology': return m.analysis?.typology.toLowerCase().includes(term) || false;
        case 'status':
        ...
        default: // Smart Search
            ...
            return m.title.toLowerCase().includes(term) || dateStr.includes(term) || 
                   m.analysis?.suggestedSeries?.toLowerCase().includes(term) || false;
        }
    })
    .sort((a, b) => {
                     ...
                     <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-3 text-xs font-serif text-white/50">
                            ...
                            {analysis?.typology && (
                                <>
                                    <span>|</span>
                                    <span className="text-copper-400 font-bold">{analysis.typology}</span>
                                </>
                            )}
                        </div>
                    </div>
                                        ...
                                        <div className="space-y-6">
                                            <section>
                                                ...
                                                <div className="bg-white p-4 rounded border border-wood-800/10 space-y-4">
                                                    <div>
                                                        ...
                                                        <span className="font-display font-bold text-lg text-wood-900">{analysis.typology}</span>
                                                    </div>
                                                    <div>
                                                        <span className="block text-xs text-wood-800/60 uppercase">Serie Archivística</span>
                                                        <span className="font-serif font-bold text-wood-900">{analysis.suggestedSeries}</span>
                                                    </div>
                                                </div>
                                            </section>
                                            ...
                                            <section>
                                                ...
                                                <div className="space-y-2 font-serif text-sm text-wood-900">
                                                    <div className="flex justify-between border-b border-wood-800/5 pb-2">
                                        ...
                                        <div className="space-y-6">
                                            <section>
                                                ...
                                                <div className="bg-white p-4 rounded border border-wood-800/10">
                                                    <p className="font-display font-bold text-wood-900 mb-2">{analysis.titleSuggestion}</p>
                                                    <p className="text-sm font-serif text-wood-800/80 leading-relaxed mb-4">{analysis.summary}</p>
                                                    <div className="flex flex-wrap gap-2">
                                                        {analysis.keywords.map((k, i) => (
                                                            <span key={i} className="px-2 py-1 bg-wood-800/5 text-wood-900 text-xs rounded border border-wood-800/10">#{k}</span>
                                                        ))}
                                                    </div>
                                                </div>
                                            </section>
                             ...
                             <div className="animate-fade-in max-w-3xl mx-auto">
                                {!analysis ? <p className="text-center text-wood-800/40 italic">Análisis no disponible.</p> : (
                                    <div className="space-y-8">
                                        <div className="bg-white p-6 rounded border border-wood-800/10">
                                            <h4 className="font-display font-bold text-wood-900 mb-4 flex items-center gap-2">
                                                <Icons.Search className="w-5 h-5 text-copper-600" /> Lugares Detectados
                                            </h4>
                                            {analysis.geodata?.length > 0 ? (
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    {analysis.geodata.map((geo, i) => (
                                                        <div key={i} className="flex items-center gap-3 p-3 bg-wood-800/5 rounded hover:bg-wood-800/10 transition-colors">
                                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 
                                                                ${geo.type === 'origin' ? 'bg-copper-100 text-copper-800' : 'bg-parchment-200 text-wood-800'}`}>
                                                                <span className="font-bold text-xs">{geo.type === 'origin' ? 'O' : 'R'}</span>
                                                            </div>
                                                            <div>
                                                                <p className="font-bold text-wood-900 text-sm">{geo.place}</p>
                                                                <p className="text-[10px] text-wood-800/50 uppercase tracking-wider">{geo.type === 'origin' ? 'Lugar de Origen' : 'Referencia'}</p>
                                                            </div>
                                                        </div>
                                                    ))}
                                                </div>
                                            ) : (
                                                <p className="text-sm italic text-wood-800/40">No se extrajeron lugares explícitos del texto.</p>
                                            )}
                                        </div>
                                    ...
                                    <div className="bg-wood-800/5 rounded p-4 border-l-4 border-wood-800">
                                        ...
                                        <ul className="space-y-1">
                                            {analysis.documentReferences.map((ref, i) => (
                                                <li key={i} className="text-sm font-serif italic text-wood-800/80">"{ref}"</li>
                                            ))}
                                        </ul>
                                    </div>
                                )}
                        ...
                        <div className="hidden md:block col-span-2 font-serif text-wood-800/80 truncate">
                            {m.analysis ? (
                                <div className="flex flex-col">
                                    <span className="font-bold">{m.analysis.suggestedSeries}</span>
                                    <span className="text-xs opacity-70">{m.analysis.typology}</span>
                                </div>
